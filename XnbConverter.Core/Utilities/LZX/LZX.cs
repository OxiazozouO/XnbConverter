...
public class LZX : IDisposable
{
	 ...
	 * @returns {Number[]}
	 */
	private void Decompress(BufferReader bufferReader, int frame_size, int block_size, byte[] decompressed, int decompressed_index)
	{
		// 如果还没有读取头部，则读取头部
		...
		while (togo > 0)
		{
			...
			if (m_state.block_remaining == 0)
			{
				...
				// 读取24位无压缩字节数
				uint hi = bufferReader.ReadLzxBits(16);
				uint lo = bufferReader.ReadLzxBits(8);
	...
	private void ReadLengths(BufferReader buffer, Span<byte> table, int first, uint last)
	{
		...
		for (i = 0; i < 20; i++)
		{
			m_state.pretree_len[i] = (byte)buffer.ReadLzxBits(4);
		}
		// 从长度创建预树表
		...
		for (i = first; i < last;)
		{
			...
			switch (symbol)
			{
				...
				{
					// 读取重复相同哈夫曼符号的次数
					uint same = buffer.ReadLzxBits(1) + 4;
	...
	{
		ushort[] table = Pool.RentUShort(tableMaxLen);
		int table_index = 0;
		
		uint pos = 0;
		/* the current position in the decode table */
		uint table_mask = 1u << (int)bits;
		...
		// 循环遍历所有位位置
		for (int bit_num = 1; bit_num <= bits; bit_num++)
		{
			// 循环遍历要解码的符号
		...
		for (uint symbol = pos; symbol < table_mask; symbol++)
		{
			table[(int)symbol] = 0xFFFF;
		}
			

		// next_symbol = 长码分配的基地址
		uint next_symbol = (table_mask >> 1) < symbols ? symbols : table_mask >> 1;
		...
		// 再次循环遍历位
		for (uint bit_num = bits + 1; bit_num <= 16; bit_num++)
		{
			// 循环遍历符号范围
	...
	private uint ReadHuffSymbol(BufferReader bufferReader, ushort[] table, byte[] length, uint symbols, uint bits)
	{
		...
		// 提前查看指定的比特数
		// (>>> 0) 允许我们得到一个32位的无符号整数
		uint bit = bufferReader.PeekLzxBits(32);
    ...
    public static void Decompress(BufferReader bufferReader, int compressedTodo, int decompressedTodo)
    {
        ...
        LZX lzx = new LZX(16);
        // 完整的解压缩数组
        byte[] decompressed = Pool.RentByte(decompressedTodo);
