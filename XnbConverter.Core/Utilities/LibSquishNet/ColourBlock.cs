...
public static class ColourBlock
{
    ...
    public static void DecompressColour(Span<byte> rgba, ReadOnlySpan<byte> block, bool isDxt1)
    {
        ...
        byte[] bytes = Pool.RentByte(winSize);
        Span<byte> codes = bytes.AsSpan();
        codes.Fill(0);
        int a = Unpack565(block[0], block[1], codes);
        int b = Unpack565(block[2], block[3], codes[4..]);
        ...
        // unpack the indices
        byte[] indices = Pool.RentByte(winSize);
        ...
        for (int i = 0; i < winSize; ++i)
        {
            codes.Slice(4 * indices[i], 4).CopyTo(rgba[(4 * i)..]);
        }
