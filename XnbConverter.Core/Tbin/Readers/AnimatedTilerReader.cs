...
public class AnimatedTilerReader: BaseReader
{
    ...
    {
        AnimatedTile result = new AnimatedTile();
        ...
        for ( int i = 0; i < result._frameCount; )
        {
            char b = (char)bufferReader.ReadByte();
            result.Index.Add(b);
            switch ( b )
            {
                case 'T':
    ...
    {
        AnimatedTile input = (AnimatedTile)content;
        ...
        int p = -1;
        int index = -1;
        for (int i = 0; i < input._frameCount;)
        {
            char b = input.Index[++index];
            ...
            switch ( b )
            {
                case 'T':
                    ...
                    break;
                case 'S':
                    StaticTile staticTile = input.Frames[++p];
