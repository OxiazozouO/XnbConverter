    ...
    */
    public class BufferReader: IDisposable
    {
        ...
        public static BufferReader FormXnbFile(string filename)
        {
            ...
            BufferReader result = new BufferReader();
            //设置缓冲区流的字节顺序。
        ...
        public static BufferReader FormFile(string filename)=>
            new BufferReader(File.ReadAllBytes(filename));
        ...
        public int BitPosition
        {
            ...
            set
            {
                ...
                _bitOffset = value % 16;
                // 获取超过16位帧的位范围的字节寻找
                int byteSeek = ((value - Math.Abs(value) % 16) / 16) * 2;
                // 寻找溢出的16位帧
                Skip(byteSeek);
            }
        }
        ...
        //返回缓冲区。
        public byte[] Buffer;
        public byte[] GetBuffer() => Buffer[..BytePosition];
        ...
        {
            byte[] read = Read(sizeof(UInt16));//2
        ...
        {
            byte[] read = Read(sizeof(UInt32));//4
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(read);
        ...
        {
            byte[] read = Read(sizeof(double));//8
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(read);
        ...
        {
            byte[] buffer = Peek(sizeof(UInt16));//2
            if (_byteOrder == BIG_ENDIAN)
        ...
        {
            byte[] buffer = Peek(sizeof(UInt32));//4
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(buffer);
        ...
        {
            byte[] buffer = this.Peek(sizeof(Int16));//2
            if (this._byteOrder == BIG_ENDIAN)
                Array.Reverse(buffer);
        ...
        {
            byte[] buffer = this.Peek(sizeof(Int32));//24
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(buffer);
        ...
        */
        ...
        {
            byte[] buffer = Peek(sizeof(float));//4
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(buffer);
        ...
        {
            byte[] buffer = Peek(sizeof(double));//8
            if (_byteOrder == BIG_ENDIAN)
                Array.Reverse(buffer);
