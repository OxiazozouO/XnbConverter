...
{
	private const BindingFlags attrs = BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
	...
	public override object Read()
	{
		...
		{
			object v = (_types[i].IsValueType ? readerResolver.ReadValue(_readIndex[i]) : readerResolver.Read_Null(_readIndex[i]));
	...
	private void SetValue(object info, object result, object? v)
	{
		...
		{
			object obj = ((!(type.Name == "Nullable`1")) ? Activator.CreateInstance(type) : Activator.CreateInstance(type.GenericTypeArguments[0]));
			r = obj;
			CopyFields(v, ref r);
		}
	...
	private object? GetValue(object info, object? inp, Type resultType)
	{
		...
		Type type = ((info is PropertyInfo propertyInfo) ? propertyInfo.PropertyType : ((!(info is FieldInfo fieldInfo)) ? null : fieldInfo.FieldType));
		if (type == resultType)
		{
			return inp;
		}
		object obj = ((!(resultType.Name == "Nullable`1")) ? Activator.CreateInstance(resultType) : Activator.CreateInstance(resultType.GenericTypeArguments[0]));
		object r = obj;
	...
	private static bool IsProperties(PropertyInfo p)
	{
		...
		if (Attribute.GetCustomAttribute(p, typeof(ContentSerializerIgnoreAttribute)) is ContentSerializerIgnoreAttribute)
		{
			return false;
		}
	...
	private static bool IsField(FieldInfo p)
	{
		...
		if (!(Attribute.GetCustomAttribute(p, typeof(ContentSerializerAttribute)) is ContentSerializerAttribute) && (!p.IsPublic || p.IsInitOnly))
		{
			return false;
		}
		return true;
